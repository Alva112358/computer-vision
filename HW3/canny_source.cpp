...
void MyCanny::non_max_supp() {
    ...
    for (int c = 0; c < cols; c++) {
        result(0, c) = (unsigned char)NOEDGE;
        result(rows-1, c) = (unsigned char)NOEDGE;
    }
    for (int r = 0; r < rows; r++) {
        result(r, 0) = (unsigned char)NOEDGE;
        result(r, cols-1) = (unsigned char)NOEDGE;
    }
...
void MyCanny::edge_deleting(int len) {
    if (len == 0) {
        final_image = edge_image;
        return;
    } 
    ...
    /* Delete the edge which length is larger than len. */
...
bool MyCanny::write_pgm_image(char* filename, float sigma, float tlow, float thigh, char name[5]) {
    ...
    final_image.save(outfilename.c_str());
    return true;
}


/* Get edge image. */
CImg<unsigned char> MyCanny::getEdgeImage() {
    return this->edge_image;
}


/* Get final image. */
CImg<unsigned char> MyCanny::getFinalImage() {
