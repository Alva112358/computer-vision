...
struct Point {
    VlSiftKeypoint a;
    VlSiftKeypoint b;
    Point(VlSiftKeypoint _a, VlSiftKeypoint _b) : a(_a), b(_b) {}
};

class Panorama {
public:
    Panorama(string filename, int _fileNum);
    ~Panorama();
    ...
    /* Stitching 过程 */
    vector<int> getImageOffset(const vector<Point>& pairs, vector<int>& inliners);
    CImg<float> blendTwoImages(CImg<float>& image1, CImg<float>& image2, int offset_x, int offset_y, int min);
